...
def train_one_epoch(cfg, epoch, dataloader, model, loss_fn, device, optimizer, scheduler, scheduler_step_time, scaler):
    ...
    for _, (images, labels) in pbar:
        ...
        batch_accuracy = (
            pred_labels == labels.detach().cpu()).sum().item() / bs
...
def main():
    ...
    for fold in cfg.use_fold:
        ...
        for epoch in range(1, cfg.n_epochs + 1):
            ...
            print(
                f'    Train Loss: {train_loss:.5f}, Train acc: {train_accuracy*100:.3f}%, lr: {lr:.7f}')
            print(
                f'    Valid Loss: {valid_loss:.5f}, Valid acc: {valid_accuracy*100:.3f}%')
            print('-'*80)
